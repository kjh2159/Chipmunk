...
- `AppSettings`: persisted user preferences, including the selected UI language
...
Language changes replace the active WPF string resource dictionary and update the tray menu and view-model-generated status text without restarting the process.
...
`LocalizationService` implements `ILocalizationService` and swaps WPF `ResourceDictionary` files at runtime. The application currently includes English, Korean, Japanese, Simplified Chinese, and Spanish resources under `src/Chipmunk/Resources`.

Views use `DynamicResource` so open windows update immediately. Text generated by view models and the Windows Forms tray menu is refreshed through the `LanguageChanged` event. The selected `AppLanguage` is stored in `settings.json`; legacy settings without this property use the supported Windows UI language, or English when the system language is unsupported.
